...
import { SortableProposalBlock } from './SortableProposalBlock';
...
export function ProposalEditor({
  ...
  const getFullProposalText = () => {
    ...
    sortedBlocks.forEach(block => {
      ...
      if (block.type === 'scope' && block.checklist) {
        block.checklist.forEach(item => { text += `✓ ${item}\n`; });
      } else if (block.type === 'investment') {
        if (fullPrice) text += `Valor: R$ ${fullPrice}\n`;
        if (discountedPrice && discountedPrice !== fullPrice) text += `Valor promocional: R$ ${discountedPrice}\n`;
        if (installments && installmentValue) text += `Ou ${installments}x de R$ ${installmentValue}\n`;
  ...
  // Build a live preview proposal object
  const liveProposal = useMemo<Proposal>(() => ({
    ...
    public_url: proposal?.public_url || null,
  ...
  }), [title, clientName, companyName, city, contactEmail, contactPhone, blocks, fullPrice, discountedPrice, installments, installmentValue, paymentMethod, discountReason, validUntil, proposal, variables]);
    ...
    <div className="space-y-6 p-1">
      ...
      <Card className="bg-gradient-to-r from-primary/5 to-transparent border-primary/20">
        <CardContent className="py-4">
          ...
          <div className="flex items-center gap-2">
            <div className={cn("flex-1 h-2 rounded-full transition-colors", hasClientInfo ? "bg-primary" : "bg-muted")} />
            <div className={cn("flex-1 h-2 rounded-full transition-colors", hasBlocks ? "bg-primary" : "bg-muted")} />
            <div className={cn("flex-1 h-2 rounded-full transition-colors", hasPricing ? "bg-primary" : "bg-muted")} />
          </div>
          <div className="flex justify-between mt-2 text-xs text-muted-foreground">
            <span className={hasClientInfo ? "text-primary font-medium" : ""}>① Dados</span>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex: Proposta de Otimização Google" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div>
              ...
              <Input value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="Nome do contato" />
            </div>
            <div>
            ...
            <div>
              ...
              <Input type="email" value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} placeholder="email@exemplo.com" />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <Input value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder="Ex: dentista em campinas" />
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Descreva o serviço e o perfil do cliente..."
                  className="min-h-[80px]"
                />
                <Button onClick={handleGenerateWithAI} disabled={isGenerating || !aiPrompt.trim()} className="w-full">
      ...
      <div className="space-y-4">
        ...
        {blocks.length > 0 && (
          <Card className={cn(
            ...
            isReadyToSend ? "border-primary bg-primary/10" : "border-dashed border-muted-foreground/30 bg-muted/20"
          )}>
              ...
              {isReadyToSend ? (
                <>
                  <div className="text-center mb-3">
                    ...
                    <p className="text-sm text-muted-foreground">Salve e copie o link para enviar</p>
                  </div>
                  <div className="flex flex-col gap-2">
                    <Button onClick={async () => { await handleSave(); copyPublicLink(); }} className="w-full gap-2" size="lg">
                      <Link className="h-5 w-5" />
                      Salvar e Copiar Link
                    </Button>
                    <Button onClick={handleCopyFullProposal} className="w-full gap-2" variant="outline">
                      {copiedAll ? <><Check className="h-4 w-4 text-green-500" /> Copiado!</> : <><ClipboardCopy className="h-4 w-4" /> Copiar texto completo</>}
                    </Button>
                  </div>
                </>
              ) : (
                <div className="text-center">
                  <p className="text-sm text-muted-foreground mb-3">Complete as etapas para enviar</p>
      ...
      <Card className={cn(!hasPricing && "border-primary/30 bg-primary/5")}>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input type="number" value={fullPrice} onChange={(e) => setFullPrice(e.target.value)} placeholder="R$ 0,00" />
          </div>
          <div>
            <Label className="text-xs text-muted-foreground">Valor com Desconto</Label>
            <Input type="number" value={discountedPrice} onChange={(e) => setDiscountedPrice(e.target.value)} placeholder="R$ 0,00" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <Label className="text-xs text-muted-foreground">Parcelas</Label>
              <Input type="number" value={installments} onChange={(e) => setInstallments(e.target.value)} placeholder="Ex: 3" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Valor Parcela</Label>
              <Input type="text" value={installmentValue ? `R$ ${installmentValue}` : ''} readOnly className="bg-muted" />
            </div>
          </div>
          <div>
          ...
          <ScrollArea className="h-[200px]">
            <div className="space-y-2">
              {templates.length === 0 ? (
                ...
                <Button key={template.id} variant="outline" className="w-full justify-start h-auto py-2" onClick={() => handleApplyTemplate(template)}>
                  <div className="text-left">
                    <div className="font-medium text-sm">{template.name}</div>
                    {template.description && <div className="text-xs text-muted-foreground truncate max-w-[180px]">{template.description}</div>}
                  </div>
                </Button>
              ))}
            </div>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground mt-2">Use estas variáveis no texto para inserir dados automaticamente.</p>
        </CardContent>
      </Card>
    </div>
  );
      ...
      <div className="space-y-6">
        ...
        {showPreview && (
          <div className="border-t pt-6">
            ...
            <ProposalPreview proposal={liveProposal} isPublic />
          </div>
        )}
      </div>
    );
  ...
  if (!showPreview) {
    return <div className="max-w-4xl mx-auto">{editorContent}</div>;
  }
  ...
  return (
    <ResizablePanelGroup direction="horizontal" className="min-h-[80vh] rounded-lg border">
      ...
      <ResizablePanel defaultSize={45} minSize={25}>
        <ScrollArea className="h-[80vh]">
          <div className="p-4">
            <div className="flex items-center gap-2 mb-4 sticky top-0 bg-background z-10 pb-2 border-b">
              <Eye className="h-4 w-4 text-primary" />
              <span className="font-semibold text-sm">Preview ao Vivo</span>
              <Badge variant="secondary" className="text-xs ml-auto">Atualiza em tempo real</Badge>
            </div>
            <ProposalPreview proposal={liveProposal} isPublic />
          </div>
        </ScrollArea>
